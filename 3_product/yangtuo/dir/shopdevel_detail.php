...
chkLogin("shopdevel_detail.php");//微信登录授权
...
<body>
  ...
  <div align="center" class="head_back" onclick="location.href='shopdevel_index.php';">
...
$id = $_GET['id'];
    ...
    $sql = "select * from shopdevel where id=$id";
...
<div class="clear-fix"></div>
        <?php
            if($result->num_rows>0){
				...
				?>
                    ...
                    <div><label for="shop_count">发展门店：</label><?php echo $row['shop_count'] ?>家</div>
                    ...
                    <button class="button green btn-big" onclick="location.href='shopdevel_index.php';">返回</button>
                    </div>
              <?php  
            } else {
    ...
    <script>
        function setState(state,id){   //修改业务员状态
            $.ajax({
                type : "POST",  //提交方式
                url : "shopdevel_changeState.php",//路径
                dataType: "json",
                data : {
                    "id" : id,
                    "state" : state
        ...
        function editDetail(id){        //修改业务员资料
            ...
            $.ajax({
                type : "POST",  //提交方式
                url : "shopdevel_editDetail.php",//路径
                dataType: "json",
                data : {
                    "id" : id,
                    "name" : name,
